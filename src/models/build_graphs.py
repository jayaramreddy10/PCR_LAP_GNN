...
def build_graphs(P_np: np.ndarray, In_np: np.ndarray=None, n: int=None, n_pad: int=None, edge_pad: int=None, stg: str='near'):
    """
    ...
    if stg == 'tri':
        ...
        A = fully_connect(P_np[0:n, :], thre=0.4)
    elif stg == 'fps':
    ...
    edge_num = int(np.sum(A, axis=(0, 1)))   #get no of edges from adj matrix by summing all elements of A
...
def fully_connect(P: np.ndarray, thre=None):
    ...
    if thre is not None:
        xyz = P[:, :]
        dist = -2 * xyz @ xyz.T    # (1024, 1024)   -- matrix of euclidean dist b/w all possible pairs of features in both point clouds
        dist += np.sum(xyz ** 2, axis=-1)[:, None]    #(1024, 1)  -- sum of squared distances across each row
        dist += np.sum(xyz ** 2, axis=-1)[None, :]    #(1, 1024)  -- transpose of above
        # Note dist is still (1024, 1024)
        PP_dist_flag = dist > (thre**2)      #(1024, 1024) binary matrix
